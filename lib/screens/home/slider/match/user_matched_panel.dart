...
class _UserMatchedPanelState extends State<UserMatchedPanel> {
  ...
  Image getMuleImage(MuleData mule) {
    if (mule.profilePicture != null) {
      return Image.network(mule.profilePicture);
    } else {
      return Image.asset('assets/images/profile_picture_placeholder.png');
    }
  }

  String getFormattedPhoneNumber(MuleData mule) {
    return mule.phoneNumber.substring(0, 2) +
        " " +
        "(" +
  ...
  build(BuildContext context) {
    ...
    updateOrder(order);
    MuleData mule = order.acceptedBy;
    Image muleImg = getMuleImage(mule);
    String phoneNumber = getFormattedPhoneNumber(mule);
